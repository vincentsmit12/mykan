---
title: "BSocial + MinIO (S3)"
mode: "wide"
tag: "NEW"
---

Deploy BSocial with PostgreSQL and MinIO (S3-compatible storage) using Docker Compose, with clear steps and production notes.

## What you’ll set up

<CardGroup cols={2}>
  <Card title="BSocial" icon="globe" color="#0284c7" horizontal>
    BSocial web app (Next.js), on port 3000.
  </Card>
  <Card title="PostgreSQL 15" icon="database" color="#65a30d" horizontal>
    PostgreSQL database for BSocial.
  </Card>
</CardGroup>
...
- BSocial stores data in PostgreSQL.
- BSocial uploads files (e.g., avatars) to MinIO over the S3 API.
- The browser fetches public files directly from MinIO’s public URL.
- The Next.js image optimizer in BSocial must be explicitly allowed to fetch from your storage host.

Key domain settings:

- <code>NEXT_PUBLIC_BASE_URL</code> → the BSocial site
...
- Open local ports: 3000 (BSocial), 5432 (Postgres), 9000/9001 (MinIO)
...
<Steps>
  ...
  <Step title="Start services">
    ...
    <ul>
      <li>
        BSocial: <a href="http://kan.localtest.me:3000">http://kan.localtest.me:3000</a>
      </li>
      <li>
        MinIO Console:{" "}
  ...
  <Step title="Verify the setup">
    <ul>
      <li>Sign in to BSocial and upload an avatar (Settings).</li>
    ...
    <ul>
      ...
      <li>Ensure you’re using the latest BSocial image.</li>
    </ul>
...
<Tabs>
  ...
  <ul>
    ...
    <li>Keep <code>S3_FORCE_PATH_STYLE=true</code> for MinIO.</li>
    <li>Put BSocial and MinIO behind HTTPS with a reverse proxy (Traefik/Nginx) and valid TLS.</li>
  </ul>
  </Tab>
</Tabs>
...
<Accordion title="Next.js optimizer 400 — url parameter is not allowed">
  <ul>
    ...
    <li>Update to the latest BSocial image.</li>
  </ul>
</Accordion>
  ...
  <Accordion title="Connectivity checks">
    <ul>
      <li>The BSocial container must reach <code>S3_ENDPOINT</code>.</li>
...
- [BSocial README](https://github.com/kanbn/kan/blob/main/README.md)
- [Cloud compose reference](https://github.com/kanbn/kan/blob/main/cloud/docker-compose.yml)
- [BSocial #109 Issue](https://github.com/kanbn/kan/issues/109)
